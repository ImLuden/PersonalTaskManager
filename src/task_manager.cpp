...
void TaskManager::run()
{
    ...
    while (running)
    {
        ...
        input = get_valid_input("Choose an option: ");
        ...
        try
        {
            ...
            case 1:
                ...
                continue;
            case 2:
                if (!ask_back_to_menu("Do you want to add a new task? ('yes' to continue, 'no' to go back to menu): "))
                    add_new_task();
                continue;
            case 3:
                if (!ask_back_to_menu("Do you want to update all task statuses? ('yes' to continue, 'no' to go back to menu): "))
                    update_task_status();
                continue;
            case 4:
                if (!ask_back_to_menu("Do you want to change task status? ('yes' to continue, 'no' to go back to menu): "))
                    change_task_status();
                continue;
            case 5:
                if (!ask_back_to_menu("Do you want to edit task parameters by its number? ('yes' to continue, 'no' to go back to menu): "))
                    edit_task();
                continue;
            case 6:
                if (!ask_back_to_menu("Do you want to delete a task by its number? ('yes' to continue, 'no' to go back to menu): "))
                    delete_task();
                continue;
            case 7:
                if (!ask_back_to_menu("Do you want to delete all tasks? ('yes' to continue, 'no' to go back to menu): "))
                    delete_all_tasks();
                continue;
            case 8:
                if (!ask_back_to_menu("Do you want to exit the program? ('yes' to continue, 'no' to go back to menu): "))
                {
                    running = false;
...
bool TaskManager::go_back(const std::string& input) 
{
    // If input = "q", go back
...
void TaskManager::update_task_status()
{
    ...
    std::cout << "Task statuses have been successfully updated\n";
...
{   
    while (true)
    {
        ...
        input_task_id = toLowerAndTrim(get_valid_input("Enter the ID of the task to change (or enter 'q' to go back): "));

        if (go_back(input_task_id))       
        {
            std::cout << "Returning to previous step.\n";
            return;
        }

        task_id = std::stoi(input_task_id);
        ...
        while (true)
        {
            ...
            new_status = toLowerAndTrim(get_valid_input("Enter new status (Pending, Completed). Tap 'q' to go back: "));
...
std::string TaskManager::get_and_update_input(const std::string& prompt, const std::string& current_value) 
{
    while (true) 
    {
        ...
        std::cout << "Task have been updated successfully\n";
        return input; // Valid input received
    }
}
...
void TaskManager::edit_task()
{
    ...
    input = get_valid_input("Enter the number of the task you want to edit (q to go back): ");

    if (go_back(input)) return;

    // Convert the input to an integer
    int task_number = std::stoi(input);
...
void TaskManager::delete_task()
{
    ...
    while (true)
    {
        ...
        input = get_valid_input("Enter the number of the task you want to delete (q to go back): ");
        ...
        else
        {
            ...
            continue; // Exit loop if user chooses to cancel
