...
void TaskManager::add_new_task()
{
    ...
    while (true)
    {
        ...
        case 0: // Input task title
            std::cout << "Enter task title (or 'q' to go back): ";
            if (go_back(title, "")) return; // Exit to menu if 'q' is entered
            step++; // Proceed to the next step
            continue;

        case 1: // Input task description
            std::cout << "Enter task description (or 'q' to go back): ";
            if (go_back(description, ""))
            {
        ...
        case 2: // Input task due date
            std::cout << "Enter task due date (DD/MM/YYYY or 'q' to go back): ";
            if (go_back(due_date, ""))
            {
                step--; // Go back to the previous step
                continue;
...
TaskStatus Task::stringToTaskStatus(const std::string& statusStr) 
{
    ...
    std::string lowerStatus = toLowerCase(statusStr);
...
TaskStatus TaskManager::get_and_update_status()
{
    while (true)
    {
        ...
        std::getline(std::cin, new_status);
        new_status = toLowerCase(new_status);
        // Check if the user wants to cancel and go back
        //if (new_status == "q")
        //{
        //    std::cout << "Returning back.\n";
